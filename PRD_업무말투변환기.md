...
- [x] 프론트엔드 `index.html` 페이지가 FastAPI 서버에서 요청 되도록 정적라우팅(static routing) 기능이 있어야 합니다
...
- [ ] 백엔드와 프론트엔드 모두 Vercel에 같이 배포(Deploy) 되도록 한다
...
| 백엔드 | Python 3.14+ / FastAPI 0.136+ / Uvicorn 0.47+ | |
| AI 연동 | LangChain 1.3+ / langchain-openai 1.2+ | Upstage OpenAI 호환 API 사용 |
| AI 모델 | Upstage Solar-Pro2 | |
| 환경 변수 | python-dotenv 1.2+ | `.env` 파일 관리 |
| 버전 관리 | Git / GitHub | |
...
```bash
# Python 버전 확인 (3.11 이상 권장, 현재 3.14 사용 중)
python --version
...
1. [x] GitHub 레포지토리 생성 (`VibeCoding_Test`)
2. [x] 디렉토리 구조 생성
3. [x] `.gitignore` 작성 — `.env` 반드시 포함
4. [x] Upstage API 키 확인 및 `backend/.env` 파일 작성
5. [x] `requirements.txt`에 라이브러리의 버전을 명시하고 최신 버전을 사용한다 (FastAPI 0.136, LangChain 1.3 등)
6. [x] 가상환경 폴더(`.venv`) 생성 및 최신 패키지 설치 완료 (Python 3.14 대응)
...
4. [x] Playwright MCP를 사용한 브라우저 테스트
